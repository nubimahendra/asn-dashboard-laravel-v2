...
@section('content')
    <div class="container mx-auto px-10 py-8">
        ...
        <div class="max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 border-t-4 border-yellow-500">
            <form action="{{ route('masn.users.update', $user) }}" method="POST">
                @csrf
                @method('PUT')
                        ...
                        <div>
                            ...
                            <select id="pd_scope" name="pd_scope"
                                ...
                                @foreach($listPd as $pd)
                                    <option value="{{ $pd }}" {{ old('pd_scope', $user->pd_scope) === $pd ? 'selected' : '' }}>{{ $pd }}</option>
                                @endforeach
                            </select>
                ...
                <!-- Actions -->
                <div class="flex items-center justify-end gap-4 pt-6 mt-8 border-t border-gray-100 dark:border-gray-700">
                    <a href="{{ route('masn.users.index') }}"
