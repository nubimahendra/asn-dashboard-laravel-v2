...
<nav class="space-y-1 mb-2">
    <div>
        <a href="{{ route('siput.dashboard') }}"
            class="w-full flex items-center px-2 py-2 text-sm font-medium rounded-lg transition-colors group {{ request()->routeIs('siput.dashboard') ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/50 dark:text-blue-400' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700' }}">
            <svg class="mr-3 h-6 w-6 flex-shrink-0 transition-colors {{ request()->routeIs('siput.dashboard') ? 'text-blue-700 dark:text-blue-400' : 'text-gray-500 group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-gray-300' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div>
        <button type="button" id="menu-siput-usul-toggle"
            ...
            <svg id="menu-siput-usul-icon"
                class="sidebar-text h-4 w-4 text-gray-400 transform transition-transform duration-200 {{ request()->routeIs('siput.usul-slks.*') ? 'rotate-180' : '' }}"
                fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M19 9l-7 7-7-7" />
            </svg>
