...
class RekonIuranController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $listUpt = RefUnor::whereNotNull('nama_opd')
            ->where('nama_opd', '!=', '')
            ->select('nama_opd')
            ->distinct()
            ->orderBy('nama_opd')
        ...
        $isUptMode = UptFilterHelper::hasUptFilter($filterOpd);
        $uptGroups = $isUptMode ? UptFilterHelper::getUptListGrouped($filterOpd) : [];

        if ($isUptMode) {
            // In UPT mode, listUnor is replaced by the UPT list generated by helper
            $listUnor = collect(); // Not used in view when isUptMode is true, view uses uptGroups
        } else {
        ...
        if ($filterUnor) {
            if ($isUptMode) {
                $resolved = UptFilterHelper::resolveUptFilter($filterUnor);
        ...
        return view('admin.rekon-iuran.index', compact(
            'listOpd', 'listUpt', 'listUnor', 'listGolongan', 'listJabatan', 
            'filterOpd', 'filterUnor', 'filterGolongan', 'filterJabatan', 'search',
            'pns', 'pppk', 'pegawaiList', 'eselonMappings',
            'golonganKeys', 'eselonKeys', 'isUptMode', 'uptGroups'
        ));
    }
